...
**启停开关**（首页卡片右上角，只出现在定时 / Webhook 流程上）：停止 = 自动触发不再跑 ——
定时到点不入队、webhook 打进来回 409（拒绝会落 `webhook_deliveries` 留痕）；
**手动运行不受影响**，停下来修、修好手动验一次、再启用，是这个开关设想的用法。
停止期间调度器**只推进时刻不触发**，所以重新启用不会补跑错过的那些，
从下一个正常时点开始（`PUT /api/flows/{id}/paused`，存在 `flows.paused_at`，016 迁移）。
...
- **首页 = 流程列表**：卡片网格，带搜索、创建副本、导出 / 导入 JSON、删除；
  ...
  定时 / Webhook 流程的卡片带**启停开关**，停止只关自动触发，手动运行照常
...
- **等待节点**：固定时长或范围内随机等一段再继续，单次上限 1 小时（上限的理由见
  `WAIT_MAX_SECONDS` 的注释）。服务端等待**不占 worker**（写一行 `waiting/sleep`
  交给唤醒循环），点停止会提前唤醒，运行面板显示「等到 几点」；离线 mock 不真等
