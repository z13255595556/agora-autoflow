...
**失败时通知**：填一个企微群机器人地址，整条运行失败（不管是定时、Webhook 还是手动）就往群里发一条；
同一原因 10 分钟内只发一条，发不出去不影响运行状态（告警是运行的旁路，`worker/alerts.ts`）。
告警里的「运行详情」链接要能点开，得给 worker 配 `PUBLIC_APP_URL`（前端的对外地址，见 `deploy/env.example`）——
